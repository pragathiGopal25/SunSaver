package no.uio.ifi.in2000.team54.ui.theme

import androidx.compose.ui.graphics.Color


val Light = Color(0xFFFCF7F7)
val Lighter = Color(0xFFFDE9B4)
val LightestYellow = Color(0xFFFAE9B3)
val LightYellow = Color(0xFFF9D695)
val LightOrange = Color(0xFFF9D694)
val DarkYellow = Color(0xFFE9BA0C)
val YellowBorder = Color(0xFFF2B94A)
val YellowText = Color(0xFFEABA0E)
val Beige = Color(0xFFFFD5B0)
val DarkBeige = Color(0xFFF9B87D)
val BrightYellow = Color(0xFFFFDA53)
val YellowNav = Color(0xFFF0C571)
val Background = Color(0xFFFFF4D1)
val GreyText = Color(0xFF646262)
val RandomBeige = Color(0xFFF0C571)
val Red = Color(0xFFBC3333)
val SoftRed = Color(0xFFDE9B9B)
val YellowerBorder = Color(0xFFFFB954)
val Butter = Color(0xFFFFE7AC)
val SupplierYellow = Color(0xFFFFEECE)
val SolcelleCard = Color(0xFF27493B)
val FjordkraftOrange = Color(0xFFFF5600)
val LightGrey = Color(0xFFE4E0E0)
